...
\end{equation}
where $\mathbf{x}^\gamma  = \texttt{lcm}(\mathrm{LM}(f), \mathrm{LM}(g))$ and $\mathbf{c} = \texttt{lcm}(\lc(f), \lc(g))$. The Gr\"obner basis method is a well-known technique for solving a system of polynomials. For a complete introduction to the topic of Gr\"obner basis we refer the reader to~\cite{Cox2015Ideals}.
