...
standard integer arithmetic, $B(\log q) \ll \log q$, while fast integer arithmetic allows $B(\log q) \ll M(\log q) \log q \log \log q$, where $M(t)$ is the complexity of multiplying two $t$-bit integers~\cite{DBLP:conf/issac/Storjohann96}.

Let \(\ell_{\texttt{prob}}\) be the number of variables in the polynomial system corresponding to the problem \texttt{prob} $\in \{\text{LWE}, \rlwe, \mlwe\}$, namely \(\ell_{\text{LWE}} = n\), \(\ell_{\rlwe} = N\), and \(\ell_{\mlwe} = Nn\). The complexity of our algorithm for solving the three LWE variants with bounded errors (e.g.\ error entries/coefficients drawn from a set of size $d$) is \[\mathscr{C}^{(q)}_{alg} = \bigO\left(\ell_{\texttt{prob}}^{\max(3(d-1), d\omega)}B(\log q)\right)\] bit operations. 
...
This means that we can represent elements of \(\Z_{q}\) as tuples of elements in the rings \(\Z_{p_i^{e_i}}\). In particular, we can represent the LWE samples as tuples of samples in the rings \(\Z_{p_i^{e_i}}\). This allows us to apply our algorithm to each component of the tuple separately and then combine the solutions using the CRT to obtain a solution in \(\Z_{q}\).
...
When, instead, $q = \prod_{i=1}^r p_i^{e_i}$ with $e_i > 1$ for some $i$, applying the algorithm over \(\F_{p_i}\) and then lifting to higher dimensions may introduce additional overhead and complexity which could become a bottleneck in the overall algorithm. In this case, a better approach is to directly work over the rings \(\Z_{p_i^{e_i}}\). If \(\sum_{i=1}^r \log_2(p_i^{e_i}) \ll \log_2(q)\), due to the smaller size of the rings \(\Z_{p_i^{e_i}}\) compared to \(\Z_q\), repeating the algorithm $r$ times could be more efficient than working directly over \(\Z_q\).
