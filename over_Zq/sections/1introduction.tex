...
% \begin{definition}[Ring-LWE~\cite{EC:LyuPeiReg10} and Module-LWE~\cite{EPRINT:BraGenVai11}]
...
% If $n=1$, we refer to the problem as Ring-LWE (\rlwe), and if $n>1$, we refer to it as Module-LWE (\mlwe).
% \end{definition}
...
%     Let $N \in \Z_{>0}$ (usually a power of $2$), $q, n \in \Z_{>0}$, $R_q = \Z_q[x]/\langle \Phi_N(x) \rangle$ with $\Phi_N(x)$ an $N$-degree irreducible polynomial, and let $\chi$ be a probability distribution on $\Z$ (for LWE) or $R_q$ (for Ring-LWE and Module-LWE).
