...
\begin{definition}[LWE~\cite{EPRINT:AlbPlaSco15,STOC:Regev05}]\label{def:LWE}
    Let $n$, $q$ be positive integers, $\chi$ be a probability distribution on $\mathbb{Z}$ and $\mathbf{s}$ be a secret vector in $\mathbb{Z}_q^n$.
    ...
    \begin{itemize}
        \item Decision-LWE is the problem of deciding whether pairs $(\mathbf{a},c)\in\mathbb{Z}_q^n\times\mathbb{Z}_q$ are sampled according to $L_{\mathbf{s},\chi}$ or the uniform distribution on $\mathbb{Z}_q^n\times\mathbb{Z}_q$.
        \item Search-LWE is the problem of recovering $\mathbf{s}$ from $(\mathbf{a},c) = (\mathbf{a}, \langle \mathbf{a}, \mathbf{s} \rangle + e)\in\mathbb{Z}_q^n\times\mathbb{Z}_q$ sampled according to $L_{\mathbf{s},\chi}$.
    \end{itemize}
  \end{definition}


\begin{definition}[RingLWE~\cite{EC:LyuPeiReg10}]
Let $n \in \mathbb{N}$ (usually a power of $2$), $q \in \mathbb{Z}^+$, and let
$R_q = \mathbb{Z}_q[x]/\langle \Phi_n(x) \rangle$
be the quotient ring, where $\Phi_n(x) = x^n + 1$ is the $2n$-th cyclotomic polynomial.
Let $\chi$ be an error distribution over $R_q$.
For a secret $s \in R_q$, the Ring-LWE distribution
$A^R_{s,\chi}$ produces samples $(a, b) \in R_q \times R_q$ where:
\[
    a \xleftarrow{\$} R_q, \quad e \leftarrow \chi, \quad b = a \cdot s + e \in R_q.
\]
The \emph{search} variant asks to recover $s$ given arbitrarily many samples from
$A^R_{s,\chi}$. The \emph{decision} variant asks to distinguish
$A^R_{s,\chi}$ from the uniform distribution over $R_q \times R_q$.
\end{definition}

\begin{definition}[ModuleLWE~\cite{EPRINT:BraGenVai11}]
    Let $n \in \mathbb{N}$ (usually a power of $2$), $k, q \in \mathbb{Z}^+$, and let
$R_q = \mathbb{Z}_q[x]/\langle x^n + 1 \rangle$.
Let $\chi$ be an error distribution over $R_q$.
For a secret vector $\mathbf{s} \in R_q^k$, the Module-LWE distribution
$A^M_{\mathbf{s},\chi}$ produces samples $(\mathbf{a}, b) \in R_q^k \times R_q$ where:
\[
    \mathbf{a} \xleftarrow{\$} R_q^k, \quad e \leftarrow \chi, \quad
    b = \langle \mathbf{a}, \mathbf{s} \rangle + e \in R_q.
\]
The \emph{search} variant asks to recover $\mathbf{s}$ given arbitrarily many samples from
$A^M_{\mathbf{s},\chi}$. The \emph{decision} variant asks to distinguish
$A^M_{\mathbf{s},\chi}$ from the uniform distribution over $R_q^k \times R_q$.
\end{definition}
...
We consider algebraic attacks on LWE instances with \emph{bounded errors}, which asks to recover $\mathbf{s}$ from $(\mathbf{a}_i, b_i) = (\mathbf{a}_i, \langle\mathbf{a}_i, \mathbf{s}\rangle + e_i) \in \mathbb{F}_q^n \times \mathbb{F}_q$, for $1 \leq i \leq m$, where each $e_i$ lies in a fixed set of size $d$.
...
The complexity of solving a \slwe\ instance (for bounded errors) with parameters $n$, $d$ $(<n)$ and $m = \binom{n+d-1}{d}$ samples is $\mathcal{O}\!\left(\binom{n+d}{d}^3\right)$, measured in the number of field operations in $\mathbb{F}_q$, where $n$ denotes the length of the secret vector to be recovered and $d$ the size of the set from which the error is drawn.
\end{theorem}
...
%For applying Gr{\"o}bner algorithm, or its variants, one should work over $\mathbb{F}_q$
