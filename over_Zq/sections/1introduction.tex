...
\begin{definition}[RingLWE~\cite{EC:LyuPeiReg10} and ModuleLWE~\cite{EPRINT:BraGenVai11}]
    Let $N \in \mathbb{N}$ (usually a power of $2$), $q \in \mathbb{Z}^+$, 
$R_q = \mathbb{Z}_q[x]/\langle \Phi_N(x) \rangle$, where $\Phi_N(x) = x^N + 1$ is the $N$-th cyclotomic polynomial, and let $\chi$ be an error distribution over $R_q$.
For a secret vector $\mathbf{s} \in R_q^k$, the distribution
...
If $k=1$, we refer to the problem as RingLWE, and if $k>1$, we refer to it as ModuleLWE.
\end{definition}
...
We consider algebraic attacks on LWE instances with \emph{bounded errors}, which asks to recover $\mathbf{s}$ from $(\mathbf{a}_i, b_i) = (\mathbf{a}_i, \langle\mathbf{a}_i, \mathbf{s}\rangle + e_i) \in \Z_q^n \times \Z_q$, for $1 \leq i \leq m$, where each $e_i$ lies in a fixed set of size $d$.
...
Each sample $(\mathbf{a}_i, b_i) = (\mathbf{a}_i, \langle\mathbf{a}_i, \mathbf{s}\rangle + e_i) \in \Z_q^n \times \Z_q$, for $1 \leq i \leq m$, allows to generate a non-linear equation of degree $d$ in the $n$ components of the secret $\mathbf{s}$. %which holds with probability $1 - e^{\mathcal{O}({-t^2})}$.
