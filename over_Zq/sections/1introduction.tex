...
% -------------------------------------------
\begin{definition}[LWE~\cite{EPRINT:AlbPlaSco15,STOC:Regev05}, Ring-LWE~\cite{EC:LyuPeiReg10} and Module-LWE~\cite{EPRINT:BraGenVai11}]
    Let $N, n, q \in \Z_{>0}$, $R_q = \Z_q[x]/\langle h_N(x) \rangle$, where $h_N(x)$ is an irreducible polynomial of degree $N$, and let $\chi$ be an error distribution over $R_q$.
    ...
    \]
    The \emph{search} variant asks to recover $\vars{s}$ given arbitrarily many samples from $A_{\vars{s},\chi}$. The \emph{decision} variant asks to distinguish $A_{\vars{s},\chi}$ from the uniform distribution over $R_q^n \times R_q$.
    In cryptographic applications, usually $h_N(x)=x^N+1$ where $N$ is a power of $2$.
...
\textbf{Remark.} While the original \rlwe\ problem~\cite{EC:LyuPeiReg10} was defined over number fields, the formulation over $R_q$ is sometimes referred to as Polynomial-LWE (PLWE). The two problems coincide for power-of-two cyclotomic rings ($h_N(x) = x^N+1$); hence, following standard cryptographic practice, we simply refer to this formulation as \rlwe. \mlf{Check if it works.}
...
% \end{table}
We propose a new algebraic algorithm for the search version of LWE (RLWE and MLWE) with \emph{bounded errors}. % which asks to recover $\vars{s}$ from $(\vars{a}_i, b_i) = (\vars{a}_i, \langle\vars{a}_i, \vars{s}\rangle + e_i) \in \Z_q^n \times \Z_q$, for $1 \leq i \leq m$, where each $e_i$ lies in a fixed set of size $d$.
It is well-known that the bound-error variant remains as hard as standard LWE for suitable parameters~\cite{MicPei13}. While previous algebraic
algorithms and their refinements uses polynomial system solving techniques that are applicable when the co-efficients are in $\F_q$, the distinctness of
our (algebraic) approach is its applicability when co-efficients are in a ring. Surprisingly, previous works do not discuss the applicability of the techniques when the polynomial coefficients are in a ring, although it is well-known that it is non-trivial to extend the well-known polynomial system solving techniques such as Gr\"obner basis, when the coefficients are elements of a ring (e.g. $\Z_q$). 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ c | c | c c | c c }
        \hline\hline
        \multirow{2}{*}{Variant} & \multirow{2}{*}{\makecell{Secret\\Size}} & \multicolumn{2}{c|}{This Work} & \multicolumn{2}{c}{Our Ring Extension of Arora and Ge~\cite{ICALP:AroGe11}} \\
        \cline{3-6}
...
All the above-mentioned algebraic analysis using polynomial system solving methods, work over $\F_q$. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


% \paragraph{Algorithms for RLWE/PLWE}
% Because an RLWE sample can be naturally represented as $N$ standard LWE samples, any known attack against LWE can be directly applied to the RLWE setting. However, the added algebraic structure of polynomial rings introduces unique attacks that do not exist in standard LWE.
% A primary class of these algebraic attacks exploits ring homomorphisms. Initially, it was demonstrated that PLWE exhibits weak instances when the defining polynomial satisfies $h_N(1) \equiv 0 \pmod q$. By ``guessing'' the evaluation of the secret polynomial at 1, an attacker can solve the Decision PLWE problem in $\mathcal{O}(q)$ time. While this targets highly specific and non-standard instances, subsequent work generalized this vulnerability to the broader RLWE problem. Specifically, researchers showed that by choosing an alternative polynomial basis, apparently secure RLWE instances can be mapped into these weak PLWE instances. If the error distortion during this transformation is sufficiently small, the $\mathcal{O}(q)$ attack remains effective. 
...
\paragraph{Algorithms for RLWE/PLWE}
Since an \rlwe\ sample naturally translates into $N$ standard LWE samples, any known LWE algorithm can be directly applied. However, the polynomial ring structure introduces unique algebraic vulnerabilities. A primary class of attacks exploits ring homomorphisms to map seemingly secure instances into weak finite fields, typically when the defining polynomial has small roots, e.g., $h_N(1) \equiv 0 \pmod q$~\cite{EPRINT:EliasLP14}. By mapping \rlwe\ instances into these weak PLWE instances~\cite{EPRINT:EliasLP15}, attackers can solve both the Decision and Search variants in $\bigO(q)$ time using statistical tests on the mapped errors~\cite{SIAM:Chen16}. To preclude these $\bigO(q)$ algebraic attacks, modern cryptographic schemes strictly employ 2-power cyclotomic rings. In this setting, such subfield vulnerabilities do not exist and the \rlwe\ and PLWE problems are proven to be polynomially equivalent~\cite{EPRINT:RoscaS18}. \mlf{References to be added.}
