...
\algdiag\ transforms a set of polynomials in Hermite form $\mathcal{P}_i$ into diagonal form $\mathcal{D}_i$ by iteratively eliminating leading terms from the remaining polynomials.
...
% \begin{algorithm}[t!]
% \caption{(\algdiag\) --- Diagonalization: conversion to Hermite form}\label{alg:step2b}
% \begin{algorithmic}[1]
...
\begin{theorem}\label{thm:iterationCost}
    In \Cref{alg:alg} the complexity due to $\algspoly$, $\algdiag$ are as follows:
    \begin{enumerate}
        \item $\mathscr{C}_{\algspoly} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$ over $d-1$ iterations,
        \item $\mathscr{C}_{\algdiag} = \mathscr{C}_{\texttt{HermiteForm}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$, over $d$ iterations. \luc{To be updated}
...
\begin{proof}[Proof of \Cref{thm:result}]
    ...
    \begin{align*}
        \mathscr{C}_{tot} &:= \mathscr{C}_{\algspoly} + \mathscr{C}_{\algdiag} = \mathcal{O}\left(\binom{n+d}{d}^3\right)
