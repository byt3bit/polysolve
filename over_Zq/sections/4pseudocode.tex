...
\Cref{alg:step2b} transforms a set of polynomials in Hermite form $\mathcal{P}_i$ into diagonal form $\mathcal{D}_i$ by iteratively eliminating leading terms from the remaining polynomials.
...
Consequently, recalling that $\deg(\lm(f))=\deg(\lm(g))=i+1$, both $\frac{\vars{x}^\gamma}{\lm(f)}$ and $\frac{\vars{x}^\gamma}{\lm(g)}$ (in \Cref{eq:spoly-expanded}) are monomials of degree one. Using the notation of Section~\ref{sec:background}, we decompose
\[
...
\begin{algorithm}[t!]
\caption{(\texttt{Diag}) --- Diagonalization: conversion to Hermite form}\label{alg:step2b}
\begin{algorithmic}[1]
...
\begin{theorem}\label{thm:iterationCost}
    In \Cref{alg:alg} the complexity due to $\texttt{Spoly}$, $\texttt{Diag}$ are as follows:
    \begin{enumerate}
        \item $\mathscr{C}_{\texttt{Spoly}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$ over $d-1$ iterations,
        \item $\mathscr{C}_{\texttt{Diag}} = \mathscr{C}_{\texttt{HermiteForm}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$, over $d$ iterations. \luc{To be updated}
    ...
    %     \item conversion to hermite form: $C_{\texttt{DiagA}}(i) =\sum_{j=1}^{\binom{n+i-1}{i}}\!\left[\!\left[ \binom{n+i}{i} - j + 1 \right]\cdot \!\left[ \binom{n+i-1}{i} - j + 1 \right]\right]$,
...
\begin{proof}[Proof of \Cref{thm:result}]
    ...
    \begin{align*}
        \mathscr{C}_{tot} &:= \mathscr{C}_{\texttt{Spoly}} + \mathscr{C}_{\texttt{Diag}} = \mathcal{O}\left(\binom{n+d}{d}^3\right)
