...
For $1 \le i \le d-1$, let $\mathcal{P}_i$ denote the set of $S$-polynomials generated from $\mathcal{D}_{i+1}$, where $\mathcal{P}_d$ denotes the initial sample set, and let $\mathcal{Q}_i$ denote the set obtained from $\mathcal{P}_i$ after the row-echelon transformation (\texttt{DiagA}). For each iteration-$i$, these sets have the same cardinality:
$|\mathcal{P}_i| = |\mathcal{Q}_i| = |\mathcal{D}_i| = \binom{n+i-1}{i}$.
...
\Cref{alg:step2a} converts a set of polynomials $\mathcal{P}_i$ into row-echelon form $\mathcal{Q}_i$ by iteratively normalizing leading terms and reducing the remaining polynomials.
\Cref{alg:step2b} transforms a set of polynomials in row-echelon form $\mathcal{Q}_i$ into diagonal form $\mathcal{D}_i$ by iteratively eliminating leading terms from the remaining polynomials.
...
\begin{definition}[Admissible Pair]\label{def:admissible-pair}
    Given two tuples of integers \(\alpha, \beta \in \mathbb{Z}_{\ge 0}^n\) such that \(|\alpha|=|\beta|=t\), the pair \((\alpha, \beta)\) is called an \emph{admissible pair} if and only if there exist two distinct standard basis vectors \(e_j, e_k \in \mathbb{Z}_{\ge 0}^n\) with \(j \neq k\) such that \(\alpha_j > 0\) and \(\beta = \alpha - e_j + e_k\). Equivalently, we say that two polynomials \(f, g\) with \(\mathrm{LM}(f) = \mathbf{x}^\alpha\) and \(\mathrm{LM}(g) = \mathbf{x}^\beta\) form an \emph{admissible pair} of polynomials if and only if the pair \((\alpha, \beta)\) is admissible. In other words, if \(|\alpha|=|\beta|=t\), then \(\deg(\gcd(\LM(f),\LM(g))) = t-1\).
\end{definition}
...
\For{$k\gets1$ to $\binom{n+i-1}{i}$ by $1$}
    ...
    \State{Select $(f,g) \in \mathcal{D}_{i+1}$ such that \(\LM(f) = \mathbf{x}^{{\ell}_{2k-1}}\) and \(\LM(g) = \mathbf{x}^{\ell_{2k}}\)} 
...
Consequently, recalling that $\deg(\mathrm{LM}(f))=\deg(\mathrm{LM}(g))=i+1$, both $\frac{\mathbf{x}^\gamma}{\mathrm{LT}(f)}$ and $\frac{\mathbf{x}^\gamma}{\mathrm{LT}(g)}$ (in \Cref{eq:spoly-expanded}) are monomials of degree one. Using the notation of Section~\ref{sec:background}, we decompose
\[
    f = f^{(i+1)} + f^{(i)} + \cdots + f^{(1)} + f^{(0)}.
\] 
Each polynomial in $\mathcal{D}_{i+1}$ contains a unique monomial of degree $i+1$, namely $\mathrm{LM}(f)$.
Multiplication by a variable $x_k$ therefore yields
\[
    x_k \cdot f = f^{(i+2)} + f^{(i+1)} + \cdots + f^{(1)},
\]
where $f^{(i+2)}$ consists of a single monomial, namely $x_k \cdot \mathrm{LM}(f)$.
...
\noindent
\textbf{\textsf{Existence and construction of admissible pairs.}} 
...
\begin{proof}
In order to prove this result, we first establish \textcolor{lipicsGray}{\textbf{\textsf{(i)}}} a necessary condition for the existence of a sufficient number of admissible pairs, and then show \textcolor{lipicsGray}{\textbf{\textsf{(ii)}}} that this condition is also sufficient by providing an explicit construction of such pairs.
...
\begin{romanenumerate}
    ...
    Hence, as shown in \textcolor{lipicsGray}{\textbf{\textsf{(i)}}}, when $n>i+1$ we can generate (at least) $\binom{n+i-1}{i}$ pairwise distinct \emph{admissible} pairs.
\end{romanenumerate}
\end{proof} 
...
\begin{algorithm}[t!]
\caption{(\texttt{DiagA}) --- Diagonalization: conversion to row-echelon form}\label{alg:step2a}
\begin{algorithmic}[1]
...
\Statex{\hspace*{-\algorithmicindent} \textbf{Output:} $\mathcal{Q}_i$: set of polynomials in row-echelon form}

\State{$\mathcal{Q}_i \gets \emptyset$}
\For{each polynomial $f \in \mathcal{P}_i$}
    \If{$f \neq 0$}
        \State{Normalize $f$, making it monic}
        \State{Append $f$ to $\mathcal{Q}_i$ and remove it from $\mathcal{P}_i$}
        \State{Reduce all remaining polynomials in $\mathcal{P}_i$ with respect to $f$}
    \EndIf{}
\EndFor{}
\State{\Return{$\mathcal{Q}_i$}}
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[t!]
\caption{(\texttt{DiagB}) --- Diagonalization: conversion to diagonal form}\label{alg:step2b}
\begin{algorithmic}[1]
\Statex{\hspace*{-\algorithmicindent} \textbf{Input:} $\mathcal{Q}_i$: set of polynomials, $n \geq 1$: secret's dimension, $1\leq i\leq d$: current degree}
...
\For{each polynomial $f \in \mathcal{Q}_i$ (processed in reverse order)}
    \If{$f \neq 0$}
        \State{Append $f$ to $\mathcal{D}_i$ and remove it from $\mathcal{Q}_i$}
        \State{Reduce all remaining polynomials in $\mathcal{Q}_i$ with respect to $f$}
    \EndIf{}
\EndFor{}
...
\begin{theorem}\label{thm:iterationCost}
    In \Cref{alg:alg} the complexity due to $\texttt{Spoly}$, $\texttt{DiagA}$ and $\texttt{DiagB}$ are as follows:
    \begin{romanenumerate}
        \item $\mathscr{C}_{\texttt{Spoly}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$ over $d-1$ iterations,
        \item $\mathscr{C}_{\texttt{DiagA}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$, $\mathscr{C}_{\texttt{DiagB}} = \mathcal{O}\!\left(\dbinom{n+d}{d}^3\right)$, over $d$ iterations.
    ...
    %     \item conversion to row-echelon form: $C_{\texttt{DiagA}}(i) =\sum_{j=1}^{\binom{n+i-1}{i}}\!\left[\!\left[ \binom{n+i}{i} - j + 1 \right]\cdot \!\left[ \binom{n+i-1}{i} - j + 1 \right]\right]$,
...
\begin{proof}
    ...
    The complete proof is given in \Cref{sec:cc}.    
\end{proof}
...
\begin{proof}[Proof of \Cref{thm:result}]
    The complexity $\mathscr{C}_{tot}$ of \Cref{alg:alg} follows immediately by adding the complexities of the three main procedures, as given in \Cref{thm:iterationCost}, i.e.
    \begin{align*}
        \mathscr{C}_{tot} &:= \mathscr{C}_{\texttt{Spoly}} + \mathscr{C}_{\texttt{DiagA}} + \mathscr{C}_{\texttt{DiagB}} = \mathcal{O}\left(\binom{n+d}{d}^3\right)
        %&= \mathcal{O}\left(\binom{n+d}{d}^3\right) + \mathcal{O}\left(\binom{n+d}{d}^3\right) + \mathcal{O}\left(\binom{n+d}{d}^3\right) = \mathcal{O}\left(\binom{n+d}{d}^3\right).
    \end{align*}
...
Throughout this section, we assume that the error terms satisfy the prescribed bound. Thus, the secret $\mathbf{s}$ is a common zero of all polynomials in the initial set of samples $\mathcal{P}_d$.

\begin{proposition}[Preservation of the LWE secret]\label{prop:preservation}
At each iteration-$i$ of \Cref{alg:alg}, with $1 \le i \le d-1$, the secret $\mathbf{s}$ is a common zero of all polynomials in $\mathcal{D}_i$.
\end{proposition}

\begin{proof}
\textbf{\textsf{Base case.}}  
$\mathcal{D}_d$ is obtained from $\mathcal{P}_d$ via diagonalization (\texttt{Diag}), which consists of linear combinations of polynomials in $\mathcal{P}_d$.  
Since $\mathbf{s}$ is a common zero of all polynomials in $\mathcal{P}_d$, it is also a zero of all polynomials in $\mathcal{D}_d$.

We proceed by induction on $i$, decreasing from $i=d-1$ to $i=1$.

\noindent
\textbf{\textsf{Inductive step.}}  
% Assume that $\mathbf{s}$ is a common zero of all polynomials in $\mathcal{D}_{i+1}$, for $1 \le i \le d-1$. Consider the iteration-$i$:
Assume that, at iteration-$i$, $\mathbf{s}$ is a common zero of all polynomials in $\mathcal{D}_{i+1}$.
The following steps are performed:
\begin{enumerate}
    \item \emph{$S$-polynomial generation.} Let $f,g\in\mathcal{D}_{i+1}$. By the inductive hypothesis, they vanish at $\mathbf{s}$. By definition (\Cref{eq:spoly-expanded}), also their $S$-polynomial vanishes at $\mathbf{s}$.
    ...
    $\mathbf{s}$, the resulting polynomials also vanish at $\mathbf{s}$.
\end{enumerate}
Therefore, $\mathbf{s}$ is a common zero of all polynomials in $\mathcal{D}_i$, for all $1 \le i \le d$.
\end{proof}
...
% At the last iteration, the secret $\mathbf{s}$ is a common zero of the linear equations in $\mathcal{D}_1$.
% Each equation, due to diagonalization process, is of the form $ x_i - s_i$ for $i \in \{1,\dots,n\}$, and the secret is uniquely determined as $\mathbf{s} = (s_1, \dots, s_n)$.
% \end{corollary}

% \begin{proof}
% By \Cref{prop:preservation}, the property of vanishing at $\mathbf{s}$ is preserved throughout all iterations.
% At the last iteration, all polynomials have degree $1$, and in particular they are linear and monic.
% Each polynomial is therefore of the form $x_i - s_i$ for some $i$, and the secret vector $\mathbf{s}$ is uniquely recovered as $\mathbf{s} = (s_1, \dots, s_n)$.
% \end{proof}


By \Cref{prop:preservation}, $\mathbf{s}$ remains the zero of the polynomials through all iterations.
In particular, at the last iteration, all polynomials in $\mathcal{D}_1$ are linear, monic and of degree $1$.
Each equation, due to diagonalization process, is of the form $x_i - s_i$ for $i \in \{1,\dots,n\}$, and the secret is uniquely determined as $\mathbf{s} = (s_1, \dots, s_n)$.
...
% Its design ensures that the LWE secret $\mathbf{s}$ is preserved, which is the unique solution of interest for the \slwe\ problem.
% \end{remark}
